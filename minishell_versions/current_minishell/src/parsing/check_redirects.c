...
/*   Updated: 2024/09/08 15:33:28 by lkhalifa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static void	print_file_error(t_cmd *cmd, char *file)
{
...
static void	handle_appendfile(t_cmd *cmd, char *file, int arg_id)
{
	int	fd;
	...
	while (arg_id < cmd->args_indexes - 2)
	{
		...
		if (check_token_and_assign(infos, actual_token, next_token, cmd) == 1)
		{
			if (ft_strcmp(actual_token, "<") == 0)
				...
				handle_appendfile(cmd, next_token, arg_id);
			if (cmd->can_access_file == 0)
				break ;
