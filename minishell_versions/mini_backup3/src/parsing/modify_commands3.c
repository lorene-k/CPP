...
// deplacer les blocs darguments dans le tableau (enleve les blocs dans le array de sarguments de la commande qui sont vide (NULL))
int resize_args_cmd(t_cmd *cmd, int new_args_indexes)
{
    ...
    // On free l'ancien array
    // i = 0;
    // while (i < initial_args_indexes - 1)
    // {
    //     if (cmd->args[i] != NULL)
    //     {
    //         free(cmd->args[i]);
...
int modify_args_cmd(t_cmd *cmd, t_infos *infos)
{
    ...
    {
        // actual_arg = ft_strdup(cmd->args[i]);
        actual_arg = replace_str_var_new(infos, cmd->args[i]); //// LEAK ICI
        if (!actual_arg)
        {           
            // Erreur malloc a MIEUX gerer
            return (-1);
        }
        ...
        {
            actual_arg = remove_quotes(cmd->args[i]); // ERREURS ICI DANS VALGRIND
            if (!actual_arg)
            {
                return (-1);
            }
