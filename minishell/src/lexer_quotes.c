...
/*   Updated: 2024/07/18 15:21:45 by lkhalifa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	handle_single_quotes(char *line, int i, t_token **token,
		int *unclosed_quote)
{
	int	j;
...
int	get_punctuation(t_token **token, char *line, int *i)
{
	...
	else if (line[*i] == '$' && (line[*i + 1] == '?' || ft_isalpha(line[*i
					+ 1])))
