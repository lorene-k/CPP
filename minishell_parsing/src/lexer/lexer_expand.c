/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   lexer_expand.c                                     :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: lkhalifa <lkhalifa@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/07/05 13:55:21 by lkhalifa          #+#    #+#             */
/*   Updated: 2024/08/16 14:11:05 by lkhalifa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/minishell.h"

static void	get_exp_value(t_data *d, char *to_expand, char **value)
{
	if (*to_expand == '?')
		*value = ft_itoa(d->status);
	else
	{
		*value = ft_strdup(getenv(to_expand));
		if (!(*value))
			*value = ft_strjoin_memory(ft_strdup("$"), ft_strdup(to_expand));
	}
	free(to_expand);
}

static void	ft_expand(t_token **token, char *to_expand, t_data *d)
{
	char	*expanded_value;

	expanded_value = 0;
	if (!to_expand)
		return ;
	get_exp_value(d, to_expand, &expanded_value);
	if ((*token)->value && expanded_value)
		(*token)->value = ft_strjoin_memory((*token)->value, expanded_value);
	else if (expanded_value)
	{
		(*token)->value = ft_strdup(expanded_value);
		free(expanded_value);
	}
}

static int	handle_non_expansion(t_token **token, char *line, int i)
{
	int	j;

	j = 0;
	while (line[i + j] && !ft_isspace(line[i + j]))
		j++;
	if ((*token)->value)
		(*token)->value = ft_strjoin_memory((*token)->value, ft_substr(line, i,
					j));
	else
		(*token)->value = ft_substr(line, i, j);
	return (j);
}

//EXPAND VARIABLES + HANDLE EXP AT END OF QUOTED STR
int	handle_expansion(t_token **token, t_data *d, int i, int *j)
{
	int	start;
	int	k;

	start = i + (*j) + 1;
	k = 0;
	if (d->line[start] == '?')
		k += 1;
	else if (ft_isalpha(d->line[start]))
	{
		while (ft_isalnum(d->line[start + k]))
			k++;
	}
	if (!check_quotes(d->line, start, k, d))
		ft_expand(token, ft_substr(d->line, start, k), d);
	else
		k = handle_non_expansion(token, d->line, start - 1);
	if ((*token)->value)
		check_type(token);
	(*j) += k;
	return (*j);
}

// HANDLE EXPANSION IF IN QUOTES
void	check_expansion(t_token **token, t_data *d)
{
	int	i;
	int	expansion;

	i = 0;
	expansion = 0;
	while (d->line[i])
	{
		if (d->line[i] == '$' && (d->line[i + 1] == '?' || ft_isalpha(d->line[i + 1])))
		{
			if (i && !expansion)
				(*token)->value = ft_strjoin_memory((*token)->value, ft_substr(d->line, 0, i));
			else if (i && expansion)
				(*token)->value = ft_strjoin_memory((*token)->value, ft_substr(d->line, expansion + 1, i - expansion - 1));
			expansion = handle_expansion(token, d, 0, &i);
		}
		else
			i++;
	}
	if (expansion && (i != expansion + 1) && i != 1)
		(*token)->value = ft_strjoin_memory((*token)->value, ft_substr(d->line,
					expansion + 1, i - expansion - 1));
	else if (!expansion && i != 1)
		(*token)->value = ft_strjoin_memory((*token)->value, d->line);
}
